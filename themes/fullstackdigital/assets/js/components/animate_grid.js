...
;(function (global){
  ...
  global.addEventListener("DOMContentLoaded", function () {
    // Remove any existing event listeners related to a previously executed script within this context
    //animateGrid();
    if (g.scrollVar !== undefined) {
      //console.log("remove event listener", g.scrollVar)
      g.removeEventListener("scroll", g.scrollVar, false);
    }
  }, false)
  animateGrid();
  
  function animateGrid () {
    //console.log("ANIMATE GRID LOADED ++++++")
    //console.log("animate grid loaded----")
    ...
    if (!document.querySelector(".work-ajax")) {
      ...
      document.querySelector("main").appendChild(ajaxContainerCreate);
    }
    ...
    let setHeightSpacerContent = (item) => {
      ...
      if (spacerLogo.src !== logoImage.src) {
        //console.log("setting src", spacerLogo.src, logoImage.src)
    ...
    g.addEventListener("scroll", g.scrollVar, false);
